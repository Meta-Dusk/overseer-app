...
def app_log(msg: str, log_type: LogType = LogType.DEFAULT) -> None:
    """Prints and writes a log resiliently."""
    line = f"[{get_date()}]\t{msg}\n"

    try:
        if sys.stdout:
            def _printf(output: str):
    ...
    except Exception: pass # Ignore print errors
...
def can_write_to_dir(path: Path) -> bool:
    ...
    try:
        ...
        app_log(f"can_write_to_dir({path}) failed: {e}", LogType.WARNING)
        return False
...
def _choose_writable_root() -> Path:
    ...
    for cand in candidates:
        if can_write_to_dir(cand):
            app_log(f"Selected config root: {cand}", LogType.GOOD)
            return cand
...
def reset_config() -> bool:
    ...
    try:
        ...
        app_log(f"Config reset to default at: {CONFIG_PATH}", LogType.GOOD)
        return True
    except Exception as e:
        app_log(f"Failed to reset config: {e}", LogType.WARNING)
        return False
...
def ensure_config_exists() -> Path:
    ...
    app_log(f"ensure_config_exists() start. BASE_DIR='{BASE_DIR}', CONFIG_ROOT='{CONFIG_ROOT}'")
    ...
    if not CONFIG_PATH.exists():
        app_log("Config file not found. Creating default one...", LogType.WARNING)
        reset_config()
    else:
        app_log(f"Config file found: {CONFIG_PATH}", LogType.GOOD)

    app_log(f"ensure_config_exists() complete. Using CONFIG_PATH={CONFIG_PATH}", LogType.GOOD)
    return CONFIG_PATH
...
def load_app_lists() -> WindowNames:
    ...
    try:
        ...
        app_log("Successfully loaded app lists from TOML.", LogType.GOOD)

        return window_names

    except Exception as e:
        app_log(f"Failed to parse config TOML: {e}", LogType.WARNING)
        return WindowNames()


# === Dev Test ===
def test():
    def print_items(label: str, list: list[str]):
