...
class WindowHelperManager:
    ...
    def _get_helper_path(self) -> Path:
        """Returns the correct path using Flet's official assets directory variable."""
        default_assets_dir = Path(__file__).resolve().parent.parent / "assets"
        assets_dir_str = os.environ.get("FLET_ASSETS_DIR", str(default_assets_dir))
        assets_dir = Path(assets_dir_str).resolve()
        
        return assets_dir / "bin" / "window_helper.exe"
            ...
            while self.running and self.process and self.process.poll() is None:
                try:
                    ...
                    try:
                        ...
                        print(f"[WindowHelperManager]: {output}")
                        continue
