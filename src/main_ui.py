...
from managers.loader import load_app_lists, reset_config, app_log
...
async def main_ui(page: ft.Page):
    ...
    async def on_close(_):
        ...
        if app_exiting:
            ...
            return
        app_exiting = True
        app_log("App is closing... Waiting for monitor to stop.")
    ...
    async def match_app_type(app_type: AppType):
        ...
        match app_type:
            case AppType.PRODUCTIVE:
                productive_time += 1
                if productive_counter_text.spans and len(productive_counter_text.spans) > 0:
                    productive_counter_text.spans[1].text = format_time(productive_time)
                    productive_counter_text.update()
                app_log(f"Incremented productive_time to: {format_time(productive_time)}")
            ...
            case AppType.DISTRACTING:
                ...
                if not page.window.maximized and popup_menu_item_fid.checked:
                    ...
                    page.window.update()
                distraction_time += 1
                if distractions_counter_text.spans and len(distractions_counter_text.spans) > 0:
                    distractions_counter_text.spans[1].text = format_time(distraction_time)
                    distractions_counter_text.update()
                app_log(f"Incremented distraction_time to: {format_time(distraction_time)}")
                await safe_sleep(1, stop_event)
    ...
    async def monitor_focus_async():
        ...
        while not stop_event.is_set():
            ...
            if title != prev_title:
                prev_title = title
                for ctrl in current_app_col.controls:
