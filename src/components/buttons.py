...
class ExitButton(PrimaryIconButton):
    ...
    async def close_window(self, e: ft.Event[ft.IconButton]) -> None:
        if not isinstance(e.page, ft.Page): return
...
class ThemeToggleButton(ft.AnimatedSwitcher):
    content: ft.Control = field(default_factory=lambda: ft.IconButton())
    ...
    def did_mount(self):
        if not isinstance(self.content, ft.IconButton): return
        self.content.icon = (
...
class SimplePopupMenuItem(ft.PopupMenuItem):
    ...
    def init(self) -> None:
        ...
        elif isinstance(self.icon, ft.IconData):
